...
int dim, listCtr, numCars, bottom, viCtr, x, y; //+3 for the borders and the newline "\n"
/*
...
int removeNode(STATE node){
	...
	for(i = 0; i < listCtr; i++){
		for(m = 0; m<= dim+2; m++){
		//	printf("Node's xy: %d%d ?= move[%d]'s xy: %d%d\n", node.thecars[m].x, node.thecars[m].y, i, move[i].thecars[m].x, move[i].thecars[m].y);
...
int goal(STATE node){
	...
	if(node.thecars[1].x == x && node.thecars[1].y == y){
		...
		printf("COST: %d\n", curr.g);
		//curr = *curr.parent;
		//	if(curr.parent == NULL)	printf("yay\n\n");
		//	scanf("%d", &mika);
		//}
